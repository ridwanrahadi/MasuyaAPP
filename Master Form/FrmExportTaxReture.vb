Public Class FrmExportTaxReture
    Sub openfile()
    ...
    Private Sub txtNoBukti_KeyDown(sender As Object, e As KeyEventArgs) Handles txtNoBukti.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            Else
                Try
                    Tabel = Proses.ExecuteQuery("SELECT Npwp, PKP, NoFp, TglFp, NoReture, tanggal, MONTH(tanggal) AS bulan, YEAR(tanggal) AS tahun, total, ppn FROM TblNotaRet where NoReture = '" & txtNoBukti.Text & "'")
                    ...
                    Else
                        With Tabel
                            ...
                            ppnbm = "0"
                        End With
                        DGV_DATA.Rows.Add("RK", npwp, pkp, kdJenis, FG, NoFP, tglfak, noretur, tglretur, bln, tahun, dpp, ppn, ppnbm)
    ...
    Private Sub BtProses_Click(sender As Object, e As EventArgs) Handles BtProses.Click
        ...
        For r As Integer = 0 To DGV.RowCount - 1
            Try
                Tabel = Proses.ExecuteQuery("SELECT Npwp, PKP, NoFp, TglFp, NoReture, tanggal, MONTH(tanggal) AS bulan, YEAR(tanggal) AS tahun, total, ppn FROM TblNotaRet where NoReture = '" & DGV.Rows(r).Cells(0).Value & "'")
                If Tabel.Rows.Count = 0 Then
                    DGV_DATA.Rows.Add("RK", "0", "0", "0", "0", "0", "0", DGV.Rows(r).Cells(0).Value, "0", "0", "0", "0", "0", "0")
                Else
                    With Tabel
                        npwp = Tabel(0).Item("Npwp").ToString.Replace(".", "").Replace("-", "")
                        pkp = Tabel(0).Item("PKP")
